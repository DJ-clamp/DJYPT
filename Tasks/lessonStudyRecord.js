...
function getUserCourses() {
  return new Promise(async (resolve, reject) => {
    try {
      const usernameArray = option.getUserDetail();
      let userCourses = [];
...
/**
 * 使用特定做数据更新
 * courseWareId=4028831b730574950173058e2f4a016b
 * duration    = 237.123
 * schedule    = 0 or  237.123
 * isDone      = false
 */
async function DummyAddUserScores(proxy) {
  return new Promise(async (resolve, reject) => {
    const usernameArray = option.getUserDetail();
    for (let key in usernameArray) {
      let name = await options.getUserDetail(
      ...
      if (score.culDay >= 10) {
        ...
        let url = usernameArray[key].studyRecord(
          "4028831b730574950173058e2f4a016b"
        );
         ...
         * 这里需要先请求一次更新进度api把当天学习的进度进行刷新(具体不详，但是如果不刷新直接请求，有可能得不到积分)
         * 方式为 先刷新进度schedule到 60 然后继续请求刷新进度到237.123为止。
         * 请求后结算分数只能先获取一个积分然后要等待3mins左右 才会继续增加分数，原因有可能服务器限制了时间
         *
         * 对于actions的定时请求，建议仅仅每天只需一次本方法即可，然后结算分数程式需要延后至少3mins后方可请求
         * 既 crond 0 0 * * * 对应更新进度业务表示从0点开始更新
         *    crond 0/10 0-5 * * * 对应结算业务  表示每10分钟更新一次从0点到5点
         *
         */
        $.log("首次更新进度");
        url = usernameArray[key].updateStudyRecord(
          "4028831b730574950173058e2f4a016b",
          "237.12",
          "60.000"
        );
        await options.updateCourseSchedule(url, proxy);
        ...
        $.log("更新进度完毕", "准备开始当日首次结算分数 +1");
        url = usernameArray[key].studyScore(
          "4028831b730574950173058e2f4a016b",
          "237.12",
          "237.12"
        );
